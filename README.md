...
**Online play** (accounts, friends, leaderboards, multiplayer) needs the small server in `server/`:

```sh
npm start   # server + app on http://localhost:8080
```

See **[docs/ONLINE.md](docs/ONLINE.md)** for how it works and how to host it.

To run just the offline app, serve the folder with any static server and open it on your phone or desktop:
...
- **Online** (with the server):
  - Accounts are just a username, with no email or password.
  - Friends show live presence and each other's streaks.
  - Global and friends-only **daily leaderboards**.
  - **Online multiplayer** for Chess, Checkers, Reversi, Connect 4 and Tic-Tac-Toe: challenge a friend or use Quick Match. Games survive app restarts.
...
- `tests/server.js` starts a real server and checks accounts, friends, leaderboards, a full match over live events, Quick Match, resigning, restarts and account deletion.
...
- **Next ideas**: Memory and card games, chess clocks for online games, push notifications for "your turn", and chat emotes
